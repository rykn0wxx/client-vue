...
const Waves = {
  ...
  clearEffect () {
    if ('ontouchstart' in window) document.body.removeEventListener('touchstart', showEffect, false)
    document.body.removeEventListener('mousedown', showEffect, false)
  }
}

export default {
  install(Vue, opts) {
    Vue.prototype.$attachWave = Waves.attach
    Vue.mixin({
      mounted () {
        Waves.displayEffect()
      },
